...
- Карточка «Толщина линии» объединена с карточкой «Связи»: теперь в одной
  ...
  `setupSlider`/`syncControlsFromP` продолжала работать без изменений.
- В превью возвращён звук ролика. Атрибут `muted` убран с `<video>`, и больше
  нет принудительного `video.muted = true` перед запуском превью после детекции.
  В playback controls добавлена кнопка-тумблер `#mute-toggle` (🔊 / 🔇) с
  i18n-ключами `mute`/`unmute` (EN + RU), подпиской на `volumechange` и
  обновлением иконки/тултипа в `updatePlaybackUi`. Калибровка FPS в
  `detectSourceFps` по-прежнему приглушает звук только на время замера.
  Аудиодорожка экспорта не затронута — она проходит через `export.js` как раньше.
...
- ✅ **Обновление устаревших подписей:** WebM codec title, экспорт без тултипа, комментарии — MediaRecorder → Mediabunny.
- ✅ **EN/RU интерфейс:** `I18N` и `TIP_TEXTS` покрывают основные подписи и справочные тултипы; переключение языка доступно из панели.
